...
function loadHeader() {
  fetch('/subPage/map/html/subpage_map_header.html')
...
function subwayBusTab(){
  let targetLink = document.querySelectorAll('.transport_title_menu a'); //링크선택
  let tabContent = document.querySelectorAll('#subway, #bus'); //탭콘텐츠
  ...
  document.getElementById('subway').style.display = 'block';
}
subwayBusTab();
...
fetch('/subPage/map/html/subpage_map_footer.html')
.then(response => response.text())
.then(data =>{
